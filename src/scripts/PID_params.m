...
% c) Posição dos Polos Desejados no plano s
...
% d) Definição de um polo adicional rápido (para um sistema de 3a ordem)
% Geralmente 5x mais rápido que a parte real dos polos dominantes.
polo_adicional = -5 * sigma_d;
fprintf('Polo adicional rápido: %.3f\n', polo_adicional);

%% 4. Cálculo da Equação Característica Desejada (DCE)
% Método 1: Usando conv() para multiplicar polinômios
...
% A equação característica do sistema com o controlador PID é:
% s^3 + (a_p + Kp_planta*Kd)s^2 + (b_p + Kp_planta*Kp)s + Kp_planta*Ki = 0
...
%% 6. Apresentação dos Ganhos do Controlador PID
disp(' ');
...
% Criação do objeto de função de transferência do PID
num_PID = [Kd, Kp, Ki];
den_PID = [1, 0]; % Polo na origem
C_PID = tf(num_PID, den_PID);

disp(' ');
disp('Função de Transferência do Controlador PID C(s):');
disp(C_PID);

%% 7. Verificação da Equação Característica em Malha Fechada
...
disp(' ');
disp('Verificação: Equação Característica do Sistema em Malha Fechada:');
fprintf('   s^3 + %.3fs^2 + %.3fs + %.3f = 0\n', MF_den_calc(2), MF_den_calc(3), MF_den_calc(4));

%% 8. Análise da Resposta ao Degrau (Opcional)
